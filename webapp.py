...
def show_teams_name():
    data1 = requests.get("https://bagmax3443.pythonanywhere.com/").json()
...
def main():
    ...
    with tab2:
        ...
        if enter1:
            response = requests.get(f"https://bagmax3443.pythonanywhere.com/api/teamvteam?team1={i}&team2={j}").json()
    ...
    with tab3:
        ...
        if enter:
            response3 = requests.get(f"https://bagmax3443.pythonanywhere.com/api/team-record?team={i3}").json()
    ...
    with tab4:
        ...
        if enter:
            response4 = requests.get(f"https://bagmax3443.pythonanywhere.com/api/team-record?team={i4}").json()
            return win_pct(response4)

    with tab5:
        response5 = requests.get(f"https://bagmax3443.pythonanywhere.com/api/batsman-names").json()
